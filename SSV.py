...
class ssvapp_tk(tk.Tk):
	...
	def initialize(self):
		...
		helpLabel.grid(column=0,columnspan=9, row=rowI, sticky="EW")
		...
		self.redrawButton = tk.Button(self.mainFrame, text="Redraw graphic", state = 'disabled',command=lambda : self.draw_fig(self.a,self.b,self.lateral_offset,self.vertical_offset,self.result.get(),self.hist2dres.get(),self.histlat.get(),self.histvert.get()) )
		...
		#row 14
		rowI=rowI+1
		videLabel5 = tk.Label(self.mainFrame)
		...
		#row 18
		rowI=rowI+1
		videLabel7 = tk.Label(self.mainFrame)
		videLabel7.grid(column=0, row=rowI, sticky="EW")
		...
		self.computeButton.grid(column=0, columnspan=4, row=rowI)
	...
	def draw_fig(self,a,b,lateral_offset,vertical_offset,result,hist2dres=None,xbinwidth=None,ybinwidth=None):
		...
		#rect_cb = [left, bottom, 0.15, height]
		...
		bin_area= int(1.5*math.sqrt(3)*((max(lateral_offset)-min(lateral_offset))/(2*hist2dres))**2)
	...
	def launch(self) :
		...
		if self.check.get() == 0:
			if self.DEMFile.get() == "" :
				...
				self.erreur('Please select backup directory.')
			else :
				#apparent offset measurements
